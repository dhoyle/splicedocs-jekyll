...
3.  Compute the split row keys for the table:
    ...
    a.  Find primary key values that can horizontally split the table
        into roughly equal sized partitions.
    ...
    b.  Specify the column names in the csv file in the `columnList`
    ...
    c.  Invoke `SYSCS_UTIL.COMPUTE_SPLIT_KEY` to compute hbase split row
...
4.  Set up the table splits in your database:
    ...
    a.  Use `SHOW TABLES` to discover the conglomerate ID for the
    ...
    b.  Now use those values in a call to our system procedure to split
...
5.  Compute the split keys for your index:
    ...
    a.  Find index values that can horizontally split the table into
        roughly equal sized partitions.

    b.  For this example, we provide 2 index values in a file named
    ...
    c.  Specify the column names in the csv file in the `columnList`
    ...
    d.  Invoke `SYSCS_UTIL.COMPUTE_SPLIT_KEY` to compute hbase split row
        keys and write them to a file:
