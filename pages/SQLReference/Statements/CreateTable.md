...
The `IF NOT EXISTS` clause checks for tables with the same name. If no table with the same name is found, the query is executed and a new table is created. If a table with the same name exists, the query is not executed and a warning appears.  
