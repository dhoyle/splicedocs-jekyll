...
* [Examples using Spark Submit](#sparksubmit)
...
A collection of (key, value) pairs specifying the import options. For example, you can specify that sampling is not to be used with a statement like this:
  ...
  <div class="Example" markdown="1"><pre>
...
A sequence (comma-separated list) of XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX.
{: .paramDefnFirst}

createTableOptions
{: .paramName}

A string that specifies the table options XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX.
{: .paramDefnFirst}
</div>
...
<div class="preWrapperWide" markdown="1"><pre>
    ...
    if (SpliceContext.tableExists(Splice MachineTableName)) {
        SpliceContext.deleteTable(Splice MachineTableName) }
    }</pre>
{: .Example}
</div>
