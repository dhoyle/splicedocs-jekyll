...
<table summary="Snapshot isolation example transactional timeline">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>T3 reset (T3') starts. The starting values within its transactional context include the commits from T1 and T2: <code>A=30, B=30, C=20</code>.</td>
        </tr>
        <tr>
            <td><em>t17</em></td>
            <td><code> </code></td>
...
As you can see, even when all transactions complete successfully, the end result depends on the order in which the transactions committed. And all of these results meet the Isolation guarantee. Splice Machine will abort a transaction when it detects a write-write conflict, which could break the guarantee.

The Isolation guarantee would be violated by generating an ending state that *could not be generated by serially running the transactions.* For example, this sequence of actions is not valid because there is no serial sequence that could generate an end result of `A = 20`.
